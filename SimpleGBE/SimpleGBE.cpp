...
static int __devinit igb_sw_init(struct igb_adapter *adapter)
{
	...
	adapter->max_frame_size = ((SimpleGBE*)netdev)->mtu() + ETH_HLEN + ETH_FCS_LEN +
					      VLAN_HLEN;
...
static void igb_set_rx_mode(IOEthernetController *netdev)
{
	...
	if (((SimpleGBE*)netdev)->flags() & IFF_PROMISC) {
		...
		if (((SimpleGBE*)netdev)->flags() & IFF_ALLMULTI) {
			rctl |= E1000_RCTL_MPE;
...
static int igb_set_vf_vlan(struct igb_adapter *adapter, u32 *msgbuf, u32 vf)
{
	 ...
	 * the VLAN filter set.
	 */
	if (add && (((SimpleGBE*)(adapter->netdev))->flags() & IFF_PROMISC))
...
{
/* All logic implemented in SimpleGBE::interruptOccurred */
#ifndef __APPLE__
	struct igb_adapter *adapter = data;
	struct igb_q_vector *q_vector = adapter->q_vector[0];
	struct e1000_hw *hw = &adapter->hw;
...
static inline void igb_rx_hash(struct igb_ring *ring,
			       ...
			       struct sk_buff *skb)
{
	if (((SimpleGBE*)netdev_ring(ring))->features() & NETIF_F_RXHASH)
		skb_set_hash(skb,
...
OSDefineMetaClassAndStructors(SimpleGBE, super);
...
bool SimpleGBE::setupMediumDict()
{
...
bool SimpleGBE::init(OSDictionary *properties)
{
    #ifdef APPLE_OS_LOG
    igb_logger = os_log_create("com.amdosx.driver.SimpleGBE", "Drivers");
    #endif
...
void SimpleGBE::stop(IOService* provider)
{
	DEBUGOUT("stop()\n");
...
bool SimpleGBE::getBoolOption(const char *name, bool defVal)
{
...
int SimpleGBE::getIntOption(const char *name, int defVal, int maxVal, int minVal )
{
	int val = defVal;
...
bool SimpleGBE::start(IOService* provider)
{
    u32 i;
...
//---------------------------------------------------------------------------
bool SimpleGBE::initEventSources( IOService* provider )
{
    bool result = false;
    ...
    if (msiIndex != -1) {
        ...
        interruptSource = IOInterruptEventSource::interruptEventSource(this, OSMemberFunctionCast(IOInterruptEventSource::Action, this, &SimpleGBE::interruptOccurred), provider, msiIndex);
    }
#endif

    interruptSource = IOInterruptEventSource::interruptEventSource(this,&SimpleGBE::interruptHandler,provider);
    if (!interruptSource) {
	...
	watchdogSource = IOTimerEventSource::timerEventSource(this, &SimpleGBE::watchdogHandler );
    if (!watchdogSource) {
	...
	resetSource = IOTimerEventSource::timerEventSource(this, &SimpleGBE::resetHandler );
	getWorkLoop()->addEventSource(resetSource);

	dmaErrSource = IOTimerEventSource::timerEventSource(this, &SimpleGBE::resetHandler );
...
//---------------------------------------------------------------------------
IOReturn SimpleGBE::enable(IONetworkInterface * netif)
{
...
IOReturn SimpleGBE::disable(IONetworkInterface * netif)
{
    pr_err("disable() ===>\n");
...
void SimpleGBE::systemWillShutdown(IOOptionBits specifier)
{
...
UInt32 SimpleGBE::outputPacket(mbuf_t skb, void * param)
{
	struct igb_adapter *adapter = &priv_adapter;
...
void SimpleGBE::getPacketBufferConstraints(IOPacketBufferConstraints * constraints) const
{
...
IOOutputQueue * SimpleGBE::createOutputQueue()
{
	return IOGatedOutputQueue::withTarget(this, getWorkLoop());
}

const OSString * SimpleGBE::newVendorString() const
{
	return OSString::withCString("Intel");
}

const OSString * SimpleGBE::newModelString() const
{
    if (chip_idx)
...
* intelSupportsEEE
*/
UInt16 SimpleGBE::intelSupportsEEE(struct igb_adapter *adapter)
{
    struct e1000_hw *hw = &adapter->hw;
    struct e1000_mac_info *mac = &hw->mac;
...
*/
void SimpleGBE::intelSetupAdvForMedium(const IONetworkMedium *medium)
{
        struct igb_adapter *adapter = &priv_adapter;
        struct e1000_hw *hw = &adapter->hw;
        struct e1000_mac_info *mac = &hw->mac;
...
IOReturn SimpleGBE::selectMedium(const IONetworkMedium * medium)
{
    pr_err("selectMedium()===>\n");
...
bool SimpleGBE::configureInterface(IONetworkInterface * interface)
{
    ...
    // 设置型号
    snprintf(modelName, kNameLength, "Intel %s Gbit Ethernet (laobamac)", deviceModelNames[chip_idx].name);
    setProperty("model", modelName);

	return true;
}

bool SimpleGBE::createWorkLoop()
{
...
IOWorkLoop * SimpleGBE::getWorkLoop() const
{
   return workLoop;
}
...
IOReturn SimpleGBE::getHardwareAddress(IOEthernetAddress * addr)
{
...
IOReturn SimpleGBE::setHardwareAddress(const IOEthernetAddress * addr)
{
...
IOReturn SimpleGBE::setPromiscuousMode(bool active)
{
	if(active)
...
IOReturn SimpleGBE::setMulticastMode(bool active)
{
	if(active)
...
IOReturn SimpleGBE::setMulticastList(IOEthernetAddress * addrs, UInt32 count)
{
	igb_adapter *adapter = &priv_adapter;
	struct e1000_hw *hw = &adapter->hw;
...
IOReturn SimpleGBE::getChecksumSupport(UInt32 *checksumMask, UInt32 checksumFamily, bool isOutput) 
{
	*checksumMask = 0;
	if( checksumFamily != kChecksumFamilyTCPIP ) {
		pr_err("SimpleGBE: Operating system wants information for unknown checksum family.\n");
...
//-----------------------------------------------------------------------
bool SimpleGBE::addNetworkMedium(UInt32 type, UInt32 bps, UInt32 index)
{
	IONetworkMedium *medium;
...
*/
bool SimpleGBE::intelCheckLink(struct igb_adapter *adapter)
{
    struct e1000_hw *hw = &adapter->hw;
    bool link_active = FALSE;
 ...
 */
void SimpleGBE::checkLinkStatus()
{
    struct igb_adapter *adapter = &priv_adapter;
    struct e1000_hw *hw = &priv_adapter.hw;
    u32 link;
...
void SimpleGBE::interruptOccurred(IOInterruptEventSource * src, int count)
{
	struct igb_adapter *adapter = &priv_adapter;
	struct igb_q_vector *q_vector = adapter->q_vector[0];
	struct e1000_hw *hw = &adapter->hw;
...
void SimpleGBE::interruptHandler(OSObject * target, IOInterruptEventSource * src, int count)
{
	SimpleGBE * me = (SimpleGBE *) target;
	me->interruptOccurred(src, count);
}


// corresponds to igb_watchdog_task	
void SimpleGBE::watchdogTask()
{
	struct igb_adapter *adapter = &priv_adapter;
	struct e1000_hw *hw = &adapter->hw;
	int i;
...
void SimpleGBE::updatePhyInfoTask()
{
    struct e1000_hw *hw = &priv_adapter.hw;
...
void SimpleGBE::watchdogHandler(OSObject * target, IOTimerEventSource * src)
{
	SimpleGBE* me = (SimpleGBE*) target;
	me->watchdogTask();
...
void SimpleGBE::resetHandler(OSObject * target, IOTimerEventSource * src)
{
	SimpleGBE* me = (SimpleGBE*) target;
...
IOReturn SimpleGBE::registerWithPolicyMaker ( IOService * policyMaker )
{
...
IOReturn SimpleGBE::setPowerState( unsigned long powerStateOrdinal,
...
IOReturn SimpleGBE::getMaxPacketSize (UInt32 *maxSize) const {
	if (maxSize)
...
IOReturn SimpleGBE::getMinPacketSize (UInt32 *minSize) const {
	if(minSize)
...
IOReturn SimpleGBE::setMaxPacketSize (UInt32 maxSize){
	pr_err("SimpleGBE::setMaxPacketSize(%d)\n",(int)maxSize);
...
IOReturn SimpleGBE::setWakeOnMagicPacket(bool active)
{
	igb_adapter *adapter = &priv_adapter;
	if(active){
...
IOReturn SimpleGBE::getPacketFilters(const OSSymbol * group, UInt32 * filters) const {
...
UInt32 SimpleGBE::getFeatures() const {
...
void SimpleGBE::startTxQueue()
{
    DEBUGFUNC("SimpleGBE::startTxQueue\n");
...
void SimpleGBE::stopTxQueue()
{
    pr_debug("SimpleGBE::stopTxQueue()\n");
	transmitQueue->stop();
	transmitQueue->flush();
	RELEASE(txMbufCursor);
}

void SimpleGBE::rxChecksumOK( mbuf_t skb, UInt32 flag )
{
	setChecksumResult(skb, kChecksumFamilyTCPIP, flag, flag );
}
	
bool SimpleGBE::carrier()
{
	return (preLinkStatus & kIONetworkLinkActive) != 0;

}
	
void SimpleGBE::setCarrier(bool stat)
{
...
void SimpleGBE::receive(mbuf_t skb)
{
	netif->inputPacket(skb, mbuf_pkthdr_len(skb));
}

void SimpleGBE::setVid(mbuf_t skb, UInt16 vid)
{
	setVlanTag(skb, vid);
}

void SimpleGBE::setTimers(bool enable)
{
    if(enable){
        if(watchdogSource)
